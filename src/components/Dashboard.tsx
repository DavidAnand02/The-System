...
const Dashboard: React.FC<DashboardProps> = React.memo(({ 
  ...
  const { level, username, stats, skills, quests } = usePlayerStore(useShallow(state => ({
  ...
  return (
    <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
          ...
          <div className="flex items-center space-x-4 sm:space-x-6 md:space-x-12 pr-0 md:pr-4">
            <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-6">
              ...
              <div className="relative group">
                <div className={`absolute inset-0 blur-2xl opacity-50 ${rank.color.replace('text-', 'bg-')} transition-opacity duration-500 group-hover:opacity-80`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-8">
        ...
        {/* STATS PANEL */}
        <section className="lg:col-span-4 min-w-0 self-start">
        ...
        {/* ACTIVE QUESTS PANEL */}
        <section className="lg:col-span-5 min-w-0 self-start">
            ...
            <div className="flex flex-col gap-3 sm:gap-4">
              {activeQuests.length > 0 ? (
                activeQuests.map(quest => (
                  <QuestMiniCard key={quest.id} quest={quest} />
                ))
              ) : (
        ...
        {/* SKILLS PANEL */}
        <section className="lg:col-span-3 min-w-0 self-start">
...
const PanelHeader: React.FC<{ title: string; icon: React.ReactNode }> = ({ title, icon }) => (
  <div className="flex items-center justify-between px-2">
    ...
    <div className="h-[1px] flex-1 ml-8 bg-gradient-to-r from-system-accent/50 via-system-accent/10 to-transparent" />
  </div>
);

const StatMiniCard: React.FC<{ stat: StatKey; value: number }> = ({ stat, value }) => (
  <div className="bg-system-bg-panel-solid/95 backdrop-blur-xl border border-white/10 rounded-3xl p-6 hover:border-system-accent/50 transition-all duration-500 group hover-glitch cursor-pointer shadow-xl hover:shadow-system-accent/10 hover:-translate-y-1">
...
const QuestMiniCard: React.FC<{ quest: any }> = ({ quest }) => (
  <div className="bg-system-bg-panel-solid/95 backdrop-blur-xl border border-white/10 rounded-3xl p-6 hover:border-system-accent/50 transition-all duration-500 group relative overflow-hidden hover-glitch cursor-pointer shadow-xl hover:shadow-system-accent/10 hover:-translate-y-1">
...
const SkillMiniCard: React.FC<{ skill: any }> = ({ skill }) => (
  <div className="flex items-center justify-between bg-system-bg-panel-solid/95 backdrop-blur-xl border border-white/10 rounded-3xl p-5 hover:border-system-accent/50 transition-all duration-500 hover-glitch cursor-pointer shadow-xl hover:shadow-system-accent/10 hover:-translate-y-1 group">
...
const MenuButton: React.FC<{ label: string; onClick: () => void; icon: React.ReactNode }> = React.memo(({ label, onClick, icon }) => (
  <button 
    onClick={onClick}
    className="flex flex-col items-center justify-center space-y-4 bg-system-bg-panel-solid/95 backdrop-blur-2xl border border-white/10 p-8 rounded-[2.5rem] text-system-text-muted hover:text-system-accent hover:border-system-accent/50 hover:bg-system-accent/10 transition-all duration-500 group hover-glitch shadow-2xl hover:shadow-system-accent/15 hover:-translate-y-2"
  >
